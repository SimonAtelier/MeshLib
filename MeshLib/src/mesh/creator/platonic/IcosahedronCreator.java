...
public class IcosahedronCreator implements IMeshCreator {
	...
	private static final int[][] FACES = { { 2, 0, 1 }, { 1, 0, 5 },
			{ 3, 0, 2 }, { 4, 0, 3 }, { 5, 0, 4 }, { 1, 5, 10 }, { 2, 1, 6 },
			{ 3, 2, 7 }, { 4, 3, 8 }, { 5, 4, 9 }, { 10, 6, 1 }, { 6, 7, 2 },
			{ 7, 8, 3 }, { 8, 9, 4 }, { 9, 10, 5 }, { 6, 10, 11 },
	...
	private void createVertices() {
		for (int i = 0; i < VERTICES.length; i++) {
			mesh.add(new Vector3f(VERTICES[i][0], VERTICES[i][1],
					VERTICES[i][2]));
		}
	}
	...
	private void createFaces() {
		for (int i = 0; i < FACES.length; i++) {
			mesh.add(new Face3D(FACES[i][0], FACES[i][1],
					FACES[i][2]));
		}
	}
